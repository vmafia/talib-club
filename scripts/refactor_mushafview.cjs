...
fs.writeFileSync('src/pages/quran/components/MushafView.jsx', modalCode);
console.log('Saved MushafView.jsx');
...
// 1. Replace the first block (pageVerses)
const block1Start = '                          <div\\n                            className="mushaf-flow"';
const block1End = '                            )}\\n                          </div>';

// use regex because spaces might be slightly different
const regex1 = /<div\s+className="mushaf-flow"[\s\S]*?ไม่พบข้อมูลหน้านี้ กรุณาลองเลือกหน้าใหม่อีกครั้ง[\s\S]*?<\/div>\s*<\/div>/;
const toReplace1 = quranCode.match(regex1)?.[0];
if (toReplace1) {
  // Wait, my regex included an extra </div>, let's just do it cleanly
}
